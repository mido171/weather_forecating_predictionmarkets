...
DEFAULT_TOKEN_FILE = REPO_ROOT.parents[2] / ".secrets" / "gribstream_api_token.txt"
DEFAULT_OUTPUT_ROOT = (
    REPO_ROOT
    / "artifacts"
    / "klga_tmax"
    / "gribstream_runs_fastpath_smoke"
)
...
def load_token() -> str:
    if DEFAULT_TOKEN_FILE.exists():
        ...
        if token:
            return token
    for key in ("GRIBSTREAM_API_TOKEN", "GRIBSTREAM_API_KEY"):
        ...
        if value:
            ...
            if value:
                return value
    raise RuntimeError(
        "GribStream token not found in GRIBSTREAM_API_TOKEN/GRIBSTREAM_API_KEY "
...
def first_planned_date_by_model(database_url: str | None, job_id: str) -> dict[str, date]:
    if not database_url:
        return {}
    engine = create_engine(database_url)
    rows: dict[str, date] = {}
    with engine.begin() as conn:
        for row in conn.execute(
            text(
                """
                SELECT model_id, MIN(target_start_date) AS first_planned
                FROM audit.gribstream_backfill_chunks
                WHERE job_id = :job_id
                  AND status = 'planned'
                GROUP BY model_id
            ...
            rows[str(row["model_id"])] = row["first_planned"]
    return rows
...
def main() -> int:
    ...
    parser.add_argument("--database-url", default=os.environ.get("KLGA_DB_URL"))
    ...
    token = load_token()
    settings = load_gribstream_settings(require_api_token=False)
    model_ids = tuple(spec.model_id for spec in MODEL_SPECS) if args.models == "all" else tuple(
    ...
    first_planned = first_planned_date_by_model(args.database_url, args.job_id)
    probes: list[RunsProbe] = []
    for model_id in model_ids:
        spec = model_spec_by_id(model_id)
        smoke_start = first_planned.get(model_id) or effective_target_start(spec, cutoff_id=T1245_CUTOFF_ID)
        smoke_start = max(smoke_start, effective_target_start(spec, cutoff_id=T1245_CUTOFF_ID))
        smoke_end = min(args.end_date, smoke_start + timedelta(days=spec.default_chunk_days - 1))
