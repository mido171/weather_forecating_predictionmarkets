...
def run_named_trial(name: str, args: argparse.Namespace, experiment_dir: Path, staging_root: Path) -> dict[str, Any]:
    ...
    if name == "model_fetch_s3_w8_r4":
        return run_parallel_trial(
            ...
            workers=min(8, args.max_workers),
            range_workers=min(4, args.max_workers),
    ...
    if name == "model_fetch_s3_w16_r4":
        return run_parallel_trial(
            ...
            cycles=parse_int_list(args.cycles),
            leads=parse_int_list(args.leads),
            workers=min(16, args.max_workers),
            range_workers=min(4, args.max_workers),
            timeout=args.timeout,
            max_attempts=args.max_attempts,
            normalize=False,
    ...
    if name == "model_fetch_s3_w16_r8":
        return run_parallel_trial(
            ...
            leads=parse_int_list(args.leads),
            workers=min(16, args.max_workers),
            range_workers=min(8, args.max_workers),
            timeout=args.timeout,
            max_attempts=args.max_attempts,
            normalize=False,
            use_processes=False,
    ...
    if name == "himawari_fetch_normalize_w4":
        return run_parallel_trial(
            ...
            cycles=parse_int_list(args.cycles),
            leads=parse_int_list(args.leads),
            workers=min(4, args.max_workers),
            range_workers=1,
            timeout=args.timeout,
            max_attempts=args.max_attempts,
            normalize=True,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        choices=(1, 2),
        default=1,
        help="Global worker cap; values above two require a reviewed code change.",
    )
