...
> provenance. Current code/tests use `src` and `tests`; governed acquisition
> configuration uses `config/acquisition`; compact campaign records use
> `experiments/campaigns`; and raw objects, ledgers, and logs use the configured
> external data/run roots. Do not execute the old path examples verbatim.
