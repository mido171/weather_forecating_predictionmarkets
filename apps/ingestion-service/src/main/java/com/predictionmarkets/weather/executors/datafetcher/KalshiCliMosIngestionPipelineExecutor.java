...
public final class KalshiCliMosIngestionPipelineExecutor {
  private static final Logger logger =
      LoggerFactory.getLogger(KalshiCliMosIngestionPipelineExecutor.class);

  private KalshiCliMosIngestionPipelineExecutor() {
  }
  ...
  public static void main(String[] args) {
    try (ConfigurableApplicationContext context = new SpringApplicationBuilder(
      ...
      snapshot("Kalshi+CLI+MOS ingestion pipeline starting seriesTickers=" + seriesTickers
      ...
      try {
        ...
        snapshot("Kalshi+CLI+MOS ingestion pipeline complete.");
      } finally {
  ...
  private static void snapshot(String message) {
    String payload = "[KALSHI-CLI-MOS-PIPELINE] thread=" + Thread.currentThread().getName()
        + " " + message;
    logger.info(payload);
    ...
    for (Job job : jobs) {
      futures.add(executor.submit(() -> {
        ...
        try {
          ...
          logger.error("[KALSHI-CLI-MOS-PIPELINE] job failed {}", job.name(), ex);
        }
        return null;
      }));
    }
  ...
  private static ThreadFactory namedThreadFactory() {
    ...
    return runnable -> {
      ...
      thread.setName("kalshi-cli-mos-pipeline-" + counter.getAndIncrement());
  ...
  private static void waitAllOrFail(List<Future<?>> futures, String label) {
    ...
    if (failure != null) {
      logger.error("[KALSHI-CLI-MOS-PIPELINE] step failed (continuing). step={}", label, failure);
    }
  }
