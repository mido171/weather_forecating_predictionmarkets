...
- exclusion: any forecast features, CLMMAXT-as-canonical training labels, or market data
...
- allowed hyperparameters: six iterations, 30 second interval, three fetch attempts, two second retry sleep
...
- primary: provider-first-publication candidate count for watched date `2026-06-18`
- guardrails: immutable raw snapshot count, metadata sidecar count, watched date missing/present state, revision count
...
- correction/confirmation approach: no model selection; if row appears, later snapshots must be reviewed for revision before G1 parity can pass
