# EXP-0055 / HKG-T24-R23 Long-Form Experiment Report
...
This experiment is part of the ordered HKG T-24 Tmax research plan. The intended input families are: upper-air warmth/stability, radiation and cloud suppression, official or point-in-time TC advisory vintages, hot-tail calibrated classifiers. The scientific goal is not to make a superficial model row; it is to test a specific physical or statistical mechanism under the T-1 15:00 Asia/Hong_Kong as-of contract. The required forecast target remains the official Hong Kong Observatory Headquarters daily maximum air temperature for local day T. All predictor information must be available before the cutoff, and all model-selection decisions before R30 must use development OOF evidence only.

## Current Evidence Audited

The current repository and data root contain the following relevant evidence: HKO TC best tracks are downloaded but retrospective only, TC realtime snapshots are prospective from 2026-06-19/20, R12 radiation diagnostic exists but not promotable. This evidence is useful but insufficient for a lawful scored experiment. The data acquisition inventory says many source families were downloaded as immutable raw payloads, but a raw payload alone is not model-ready. For R13-R30, the blocker is usually not that nothing exists; it is that the source-native parser, point-in-time issue/vintage contract, station metadata reconciliation, or complete upstream expert table is not yet available.

## Precondition Gate Result

The status for HKG-T24-R23 is `BLOCKED_PRECONDITIONS_NOT_MET`. The explicit blockers are: point_in_time_tc_advisory_archive_missing, upper_air_features_missing, cloud_suppression_features_missing, hot_tail_sample_gate_not_established. Because these blockers affect the actual predictor set or validation authorization, the experiment does not train a model, does not score OOF predictions, and does not promote any feature. This is a deliberate fail-closed result. It avoids fabricating features from target-day daily climate values, retrospective best tracks, current-only snapshots, unparsed raw archives, or post-hoc validation knowledge.

## Readiness Table

| category | item | status | disposition |
| --- | --- | --- | --- |
| required_input | upper-air warmth/stability | required | not scored until available |
| required_input | radiation and cloud suppression | required | not scored until available |
| required_input | official or point-in-time TC advisory vintages | required | not scored until available |
| required_input | hot-tail calibrated classifiers | required | not scored until available |
| available_evidence | HKO TC best tracks are downloaded but retrospective only | available_or_partial | audited for blocker gate |
| available_evidence | TC realtime snapshots are prospective from 2026-06-19/20 | available_or_partial | audited for blocker gate |
| available_evidence | R12 radiation diagnostic exists but not promotable | available_or_partial | audited for blocker gate |
| blocker | point_in_time_tc_advisory_archive_missing | blocked | Build or acquire point-in-time TC advisory vintages and upper-air features before any hot-tail specialist scoring. |
| blocker | upper_air_features_missing | blocked | Build or acquire point-in-time TC advisory vintages and upper-air features before any hot-tail specialist scoring. |
| blocker | cloud_suppression_features_missing | blocked | Build or acquire point-in-time TC advisory vintages and upper-air features before any hot-tail specialist scoring. |
| blocker | hot_tail_sample_gate_not_established | blocked | Build or acquire point-in-time TC advisory vintages and upper-air features before any hot-tail specialist scoring. |
