...
def main() -> int:
    ...
    parser.add_argument("--coordinate-tier", required=True)
    ...
    parser.add_argument("--timeout-seconds", type=float, default=float(os.environ.get("GRIBSTREAM_TIMEOUT_SECONDS", "90")))
    parser.add_argument("--max-retries", type=int, default=int(os.environ.get("GRIBSTREAM_MAX_RETRIES", "2")))
    parser.add_argument("--resume", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--stop-on-local-error", action="store_true")
    parser.add_argument(
        "--execute",
        action="store_true",
        help="Acknowledge bounded provider requests and database writes.",
    )
    args = parser.parse_args()
    if not args.execute:
        parser.error("live backfill is disabled; re-run with --execute")
    if not args.database_url:
        parser.error("KLGA_DB_URL or --database-url is required")
    if args.models.strip().lower() in {"all", "*"}:
        parser.error("--models must list explicit model IDs; 'all' is not allowed")
    model_count = len(parse_model_ids(args.models))
    if args.max_models < 1 or model_count > args.max_models:
        parser.error(
            f"model scope contains {model_count} models but --max-models is {args.max_models}"
        )
    if args.end_date < args.start_date:
        parser.error("--end-date must be on or after --start-date")
    day_count = (args.end_date - args.start_date).days + 1
    if args.max_days < 1 or day_count > args.max_days:
        parser.error(
