...
EXPERIMENT_ID = "0193"
SLUG = "source_era_residual_bridge"
TITLE = "Source-Era Residual Bridge Over 0190"
EXP_DIR = EXPERIMENTS_ROOT / f"{EXPERIMENT_ID}_{SLUG}"
PRIMARY_CANDIDATE_ID = "0193_source_era_residual_bridge"
P0190 = EXPERIMENTS_ROOT / "0190_t7_post_ensemble_residual_calibration" / "predictions.parquet"
SRC_COPY_NAME = "run_0193.py"
MODEL_FOLDS = base.MODEL_FOLDS
LAG_DAYS = 7
CONFIG_GRID = [
    {"context_mode": "source", "halflife": 20.0, "min_history": 10, "shrink": 60.0, "cap_c": 0.10},
    {"context_mode": "source", "halflife": 45.0, "min_history": 20, "shrink": 100.0, "cap_c": 0.15},
    {"context_mode": "source_season", "halflife": 45.0, "min_history": 20, "shrink": 100.0, "cap_c": 0.15},
    {"context_mode": "source_month", "halflife": 45.0, "min_history": 20, "shrink": 100.0, "cap_c": 0.15},
    {"context_mode": "source_month", "halflife": 90.0, "min_history": 30, "shrink": 150.0, "cap_c": 0.18},
    {"context_mode": "source_era", "halflife": 60.0, "min_history": 25, "shrink": 120.0, "cap_c": 0.15},
    {"context_mode": "all_source_era", "halflife": 90.0, "min_history": 30, "shrink": 150.0, "cap_c": 0.18},
]
...
def load_parent() -> pd.DataFrame:
    ...
    base.assert_pre2024(frame, "0193 parent 0190 frame")
...
def contexts(row: pd.Series, mode: str) -> list[str]:
    ...
    year = int(pd.Timestamp(row.get("target_date")).year) if pd.notna(row.get("target_date")) else 0
    era = "era_2000_2004" if year <= 2004 else "era_2005_2009" if year <= 2009 else "era_2010_2014" if year <= 2014 else "era_2015_2019" if year <= 2019 else "era_2020_2023"
    out = ["global", f"source={source}"]
    if mode in {"source_season", "all_source_era"}:
        out.extend([f"source={source}|season={season}"])
    if mode in {"source_month", "all_source_era"}:
        out.extend([f"source={source}|{month}"])
    if mode in {"source_era", "all_source_era"}:
        out.extend([f"source={source}|{era}", f"source={source}|{era}|{season}"])
    if mode == "all_source_era":
        out.extend([f"source={source}|{era}|{month}", f"{era}|{month}"])
    return out
...
def prequential(frame: pd.DataFrame, config: dict[str, Any], config_id: str) -> pd.DataFrame:
    ...
    for idx, current_date in enumerate(dates):
        ...
        while add_idx < len(ordered) and dates.iloc[add_idx] <= mature_date:
            value = float(ordered.iloc[add_idx]["parent_0190_residual_c"])
    ...
    out["candidate_prediction_c"] = out["parent_0190_prediction_c"] + out["calibration_correction_c"]
...
def run_walk_forward(frame: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    out["model_family"] = "source_era_residual_bridge"
...
def build_spec(created_at: str) -> dict[str, Any]:
    return {
        ...
        "hypothesis": "The 0190 champion still has source-era residual bias that can be reduced by T-7 mature source, source-month, and source-era states.",
        "rationale": "0190 selected mostly global/month calibration and left different press versus RSS residual behavior. 0191 and 0192 showed parent-only stacking is redundant, so this experiment tests source metadata as an independent bridge.",
        "expected_sign_and_falsification": "Expected sign is lower MAE than 0190 with no tail harm. Falsified if source-era states overfit press-era quirks or damage the RSS late window.",
        "novelty": {"prior_experiments": ["0190", "0191", "0192"], "difference": "Source-era residual bridge over the 0190 champion; not correction-sign stacking or parent-only trust shrinkage.", "similarity_audit_path": "RESULTS.md#comparison-limitations"},
        ...
        "data_sources": [{"source_id": "0190_parent_predictions", "paths": [rel(P0190)], "attributes": ["candidate_prediction_c", "calibration_correction_c", "target_tmax_c for matured residual state"], "eligibility": "DEPLOYABLE_LAGGED_ONLY", "availability_proof": "0190 validator passed; residual calibration uses only rows <= T-7."}],
        "stations": [{"station_id": "HKO", "role": "target and matured residual state", "attributes": ["daily Tmax"]}],
        "features": {"generation_rule": "EW residual states by global, source, source-season, source-month, source-era, and source-era-month contexts, updated only from 0190 residuals matured by T-7.", "grid": CONFIG_GRID, "explicit_exclusions": ["2024+ rows", "current target residual", "residuals newer than T-7"]},
        "response": {"variable": "target_tmax_c - parent_0190_prediction_c", "prediction": "parent_0190_prediction_c plus clipped mature source-era residual correction"},
...
Experiment `{EXPERIMENT_ID}` is `{summary['status']}`. It is a T-7 source-era residual bridge over 0190.

## One-Sentence Hypothesis

The remaining 0190 champion residual can be reduced by mature source, source-month, and source-era states without violating T-24.

## Why It Is Worth Doing

0190 is the current comparable champion but still leaves different press versus RSS behavior. This experiment tests whether source metadata carries residual value.

## Prior Evidence And Novelty

0189 established online expert weighting, 0190 established post-ensemble calibration, and 0191/0192 ruled out parent-only stacking. 0193 tests source-era metadata as the next bridge.
...
Features are mature residual states by global, source, source-season, source-month, and source-era contexts. Details are in `feature_definitions.csv`.

## Response And Baseline

Response is parent 0190 residual. Primary baseline remains raw official forecast, with 0190 as parent reference.
...
Calibration can fail by overfitting source-era quirks or duplicating memory already inside 0190.

## Reproduction Command

Run `python scripts/run_hkg_t24_0193_source_era_residual_bridge.py` from the repository root.
""")
...
Row-level parent residual, source-era correction, and signed errors are in `predictions.parquet`.
...
This is a child calibration experiment over 0190. If it does not beat 0190, it should not replace the parent.
...
0193 tested whether 0190's remaining source-era residual bias was stable enough for a mature residual correction.
...
The result shows whether source metadata remains useful after the 0190 post-ensemble calibration.
...
If not promoted, the likely cause is that 0190's selected month/global calibration already absorbs the source-era bias.
...
If this fails, move away from source-only calibration and search for station-network independent safe signals.
""")
...
def main() -> None:
    ...
    scoreboard = pd.DataFrame(
        [
            ...
            {"candidate_id": PRIMARY_CANDIDATE_ID, "model_family": "source_era_residual_bridge", "n": candidate_global["n"], "mae_c": candidate_global["mae_c"], "rmse_c": candidate_global["rmse_c"], "bias_c": candidate_global["bias_c"], "median_abs_error_c": candidate_global["median_abs_error_c"], "p95_abs_error_c": candidate_global["p95_abs_error_c"], "gt2c_rate": candidate_global["gt2c_rate"], "gt3c_rate": candidate_global["gt3c_rate"], "baseline_mae_c": official_global["mae_c"], "mae_delta_c": mae_delta},
    ...
    data_manifest = pd.DataFrame([{"source_id": "0190_parent_predictions", "path": rel(P0190), "sha256": sha256_file(P0190), "size_bytes": P0190.stat().st_size, "row_count": int(len(frame)), "date_start": date_text(frame["target_date"].min()), "date_end": date_text(frame["target_date"].max()), "timestamp_fields": "target_date;forecast_source_family;T-7 parent residual state", "availability_class": "DEPLOYABLE_LAGGED_ONLY", "notes": "Validator-clean parent 0190 predictions."}])
    feature_definitions = pd.DataFrame([{"feature_name": "t7_parent_0190_source_era_residual_bridge", "role": "candidate_correction", "formula": "EW mean of parent 0190 residuals in predeclared source/source-season/source-month/source-era contexts; only rows with target_date <= T-7 are admitted.", "input_columns": "parent_0190_prediction_c,target_tmax_c,forecast_source_family,season,month,target_date", "units": "degC", "lag": "7 target days minimum", "window": "predeclared EW half-life grid", "fit_scope": "fold-local config selection; prequential state", "availability_rule": "No current or recent target residual updates prediction state; source and target year are known before target resolution.", "missingness_policy": "No mature state gives zero correction."}])
    pred_cols = ["target_date", "target_tmax_c", "forecast_source_family", "season", "month", "official_prediction_c", "parent_0190_prediction_c", "parent_0190_calibration_correction_c", "candidate_prediction_c", "parent_0190_residual_c", "calibration_correction_c", "candidate_correction_c", "active_calibration_context_count", "official_error_c_signed", "candidate_error_c", "official_abs_error_c", "candidate_abs_error_c", "fold_id", "selected_config_id", "candidate_id", "baseline_id", "model_family"]
...
0193 consumes validator-clean 0190 parent predictions. Source-era states use parent residuals only after target dates are at least `{LAG_DAYS}` days old. Current-row source and date metadata are known before target resolution, but the current target residual is never used.
...
```powershell
python scripts/run_hkg_t24_0193_source_era_residual_bridge.py
```
