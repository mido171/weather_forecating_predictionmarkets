# EXP-0054 / HKG-T24-R22 Long-Form Experiment Report
...
This experiment is part of the ordered HKG T-24 Tmax research plan. The intended input families are: R07 front/pressure features, R11 advection features, R14/R15 upper-air changes, R18 forecast revisions, R20/R21 regime probabilities. The scientific goal is not to make a superficial model row; it is to test a specific physical or statistical mechanism under the T-1 15:00 Asia/Hong_Kong as-of contract. The required forecast target remains the official Hong Kong Observatory Headquarters daily maximum air temperature for local day T. All predictor information must be available before the cutoff, and all model-selection decisions before R30 must use development OOF evidence only.

## Current Evidence Audited

The current repository and data root contain the following relevant evidence: R07 pressure/front proxy exists, R11/R14/R18/R20/R21 are blocked or unavailable, development-only target-change labels can be made later but predictors are incomplete. This evidence is useful but insufficient for a lawful scored experiment. The data acquisition inventory says many source families were downloaded as immutable raw payloads, but a raw payload alone is not model-ready. For R13-R30, the blocker is usually not that nothing exists; it is that the source-native parser, point-in-time issue/vintage contract, station metadata reconciliation, or complete upstream expert table is not yet available.

## Precondition Gate Result

The status for HKG-T24-R22 is `BLOCKED_PRECONDITIONS_NOT_MET`. The explicit blockers are: transition_predictor_bank_incomplete, official_forecast_revision_features_missing, upper_air_change_features_missing, regime_probabilities_missing. Because these blockers affect the actual predictor set or validation authorization, the experiment does not train a model, does not score OOF predictions, and does not promote any feature. This is a deliberate fail-closed result. It avoids fabricating features from target-day daily climate values, retrospective best tracks, current-only snapshots, unparsed raw archives, or post-hoc validation knowledge.

## Readiness Table

| category | item | status | disposition |
| --- | --- | --- | --- |
| required_input | R07 front/pressure features | required | not scored until available |
| required_input | R11 advection features | required | not scored until available |
| required_input | R14/R15 upper-air changes | required | not scored until available |
| required_input | R18 forecast revisions | required | not scored until available |
| required_input | R20/R21 regime probabilities | required | not scored until available |
| available_evidence | R07 pressure/front proxy exists | available_or_partial | audited for blocker gate |
| available_evidence | R11/R14/R18/R20/R21 are blocked or unavailable | available_or_partial | audited for blocker gate |
| available_evidence | development-only target-change labels can be made later but predictors are incomplete | available_or_partial | audited for blocker gate |
| blocker | transition_predictor_bank_incomplete | blocked | Complete required predictor families, then train nested transition classifiers and specialist corrections using development-only labels. |
| blocker | official_forecast_revision_features_missing | blocked | Complete required predictor families, then train nested transition classifiers and specialist corrections using development-only labels. |
| blocker | upper_air_change_features_missing | blocked | Complete required predictor families, then train nested transition classifiers and specialist corrections using development-only labels. |
| blocker | regime_probabilities_missing | blocked | Complete required predictor families, then train nested transition classifiers and specialist corrections using development-only labels. |
