...
README_RESULTS_START = "<!-- BEGIN GENERATED RESIDUAL ML STRATEGY RESULT -->"
README_RESULTS_END = "<!-- END GENERATED RESIDUAL ML STRATEGY RESULT -->"
...
def build_matrix(
    *,
    ...
    with psycopg.connect(database_url) as connection:
        ...
        hourly_params = {
            "hourly_start_utc": (
                pd.to_datetime(matrix["cutoff_at_utc"], utc=True).min() - pd.Timedelta(hours=36)
            ).to_pydatetime(),
            "hourly_end_utc": pd.to_datetime(matrix["cutoff_at_utc"], utc=True)
            .max()
            .to_pydatetime(),
        }
...
def write_outputs(
    *,
    ...
    presealed = matrix[
        matrix["target_date"].between(pd.Timestamp("2022-01-01"), pd.Timestamp("2023-12-31"))
    ].copy()
    ...
    for filename in (
        ...
        write_csv(
            output_dir / filename, scoreboards.get(filename.removesuffix(".csv"), pd.DataFrame())
        )
    ...
    write_json(
        ...
        {
            cutoff: payload.get("ensemble", {})
            for cutoff, payload in model_selection.get("cutoffs", {}).items()
        },
    )
    importance = run_result["feature_importance"]
    write_csv(
        output_dir / "feature_importance_lgbm.csv",
        importance[importance["model_id"].astype(str).str.contains("lgbm", case=False, na=False)],
    )
    write_csv(
        output_dir / "feature_importance_catboost.csv",
        importance[importance["model_id"].eq("M3_catboost_residual")],
    )
    write_csv(
        output_dir / "linear_coefficients.csv",
        importance[importance["model_id"].eq("M4_huber_residual")],
    )
    write_json(
    ...
    write_bounded_readme_section(
        ...
        section=(
            ...
            + "\n\n"
            + demote_markdown_headings(diagnostics).strip()
            + "\n\nMachine evidence: `results/summary.json`, `results/scoreboard.csv`, "
            "`results/leakage_audit.json`, and `results/artifact_manifest.csv`."
        ),
        default_title="Residual ML Strategy",
    )
    return summary


def run(
    config_path: Path, database_url: str, output_dir: Path, cutoff_profile_names: list[str] | None
) -> dict[str, Any]:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        default=str(
            REPO_ROOT / "experiments" / "campaigns" / "residual-modeling" / "strategy" / "results"
        ),
    )
