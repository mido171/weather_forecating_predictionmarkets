...
def load_collector_schedules(root: Path) -> dict[str, Any]:
    return load_yaml(root / "config" / "collector_schedules.yaml")
...
def run_due_schedules(root: Path) -> list[CollectOutcome]:
    due = due_schedules(root)
    if not due:
        return []
    return collect_source_ids(root, source_ids=due, continue_on_error=True)
...
def write_machine_source_catalog(root: Path) -> Path:
    data = load_yaml(root / "config" / "data_source_catalog.yaml")
...
def write_health_report(root: Path) -> Path:
    data_root = ensure_data_root(root)
    state_files = sorted((data_root / "state").glob("*.json"))
    lines = [
    ...
    for path in state_files:
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    str(state.get("source_id", path.stem)),
    ...
    report = root / "reports" / "live_collector_health.md"
...
def write_inventory_reports(root: Path) -> list[Path]:
    data_root = ensure_data_root(root)
    catalog = load_yaml(root / "config" / "data_source_catalog.yaml")
