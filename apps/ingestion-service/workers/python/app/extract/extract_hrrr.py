...
CONFIG = HerbieModelConfig(
    model="hrrr",
    product="sfc",
    default_forecast_hours=tuple(range(0, 19, 1)),
)
...
def run(request: dict) -> dict:
    ...
    request.setdefault("model_name", "hrrr")
    request.setdefault("max_forecast_hours", 18)
    return extract_with_herbie(request, "hrrr", CONFIG, VARIABLE_SPECS)
