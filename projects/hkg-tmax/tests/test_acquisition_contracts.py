...
def test_historical_live_pairs_cover_required_reset_fields(repo_root) -> None:
    ...
    data = load_yaml(repo_root / "config" / "sources" / "historical_live_pairs.yaml")
    pairs = data["pairs"]
...
def test_gridded_policy_has_required_domains_and_gated_bulk_downloads(repo_root) -> None:
    ...
    data = load_yaml(repo_root / "config" / "acquisition" / "gridded_acquisition_policy.yaml")
...
def test_historical_live_pair_artifacts_are_rendered(repo_root, monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setenv("HKG_TMAX_RUN_ROOT", str(run_root))
    paths = write_historical_live_pair_artifacts(repo_root)

    assert data_root / "metadata" / "historical_live_pairs.csv" in paths
    assert data_root / "metadata" / "historical_live_pairs.parquet" in paths
    report = run_root / "reports" / "historical_live_pairing.md"
