#!/usr/bin/env python3
"""
Download minute-by-minute Kalshi bucket prices for daily temperature event series
and export per-day CSVs.
"""
...
def _file_prefix_for_series(series: str) -> str:
    return SERIES_FILE_PREFIX.get(series.upper(), series.upper())
...
def _resolve_event_for_day(series: str, day: dt.date, client: KalshiClient) -> Tuple[Optional[str], Optional[Dict[str, Any]], List[str]]:
    """
    These daily temperature tickers can be ambiguous between DDMONYY and YYMONDD forms.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Download Kalshi daily temperature minute bucket prices.")
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "CSV filename prefix (default derived from series; e.g. "
            "KXHIGHNY->KNYC, KXHIGHMIA->KMIA)"
        ),
    )
    ...
    file_prefix = (args.file_prefix or _file_prefix_for_series(args.series)).strip().upper()
    ...
    manifest["file_prefix"] = file_prefix
    ...
    for day in _daterange(start_date, end_date):
        ...
        out_name = f"{file_prefix}_{day.strftime('%Y%m%d')}.csv"
