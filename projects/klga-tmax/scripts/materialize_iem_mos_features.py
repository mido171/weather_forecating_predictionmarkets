...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--db-url", default=os.environ.get("KLGA_DB_URL"))
    parser.add_argument("--max-days", type=int, default=31)
    parser.add_argument("--max-chunks", type=int, default=1)
    parser.add_argument(
        "--execute",
        action="store_true",
        help="Acknowledge bounded database writes after reviewing the scope.",
    )
    ...
    if not args.execute:
        parser.error("database materialization is disabled; re-run with --execute")
    if not args.db_url:
        parser.error("KLGA_DB_URL or --db-url is required")
    try:
        start = date.fromisoformat(args.start_date)
        through = date.fromisoformat(args.through_date)
    except ValueError as exc:
        parser.error(f"dates must use YYYY-MM-DD: {exc}")
    if through < start:
        parser.error("--through-date must be on or after --start-date")
    day_count = (through - start).days + 1
    if args.max_days < 1:
        parser.error("--max-days must be >= 1")
    if day_count > args.max_days:
        parser.error(
            f"date scope contains {day_count} days but --max-days is {args.max_days}"
        )
...
def reset_gold(
    ...
    cur.execute(
        """
        DELETE FROM gold.iem_mos_feature_matrix_v1
        WHERE cutoff_id = %s
    ...
    cur.execute(
        ...
        SET feature_rows_upserted = 0,
            updated_at = now()
        WHERE job_id = %s
          AND window_start_utc::date <= %s::date
          AND window_end_utc::date > %s::date
        """,
        (job_id, through_date, start_date),
    )
...
def load_chunks(
    ...
    cur.execute(
        ...
        """,
        (job_id, missing_only, through_date, start_date),
    )
...
def main() -> None:
    ...
    with psycopg.connect(args.db_url, connect_timeout=10) as conn:
        with conn.cursor() as cur:
            ...
            if args.reset:
                daily_deleted, matrix_deleted = reset_gold(
                    cur,
                    args.cutoff_id,
                    args.job_id,
                    args.start_date,
            ...
            chunks = available_chunks[: args.max_chunks]
            print(
                f"chunks_available={len(available_chunks)} "
                f"chunks_to_materialize={len(chunks)} max_chunks={args.max_chunks}"
            )
            total = 0
