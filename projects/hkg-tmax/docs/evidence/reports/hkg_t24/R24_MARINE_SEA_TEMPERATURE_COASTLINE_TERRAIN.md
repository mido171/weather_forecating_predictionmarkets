# EXP-0056 / HKG-T24-R24 Long-Form Experiment Report
...
This experiment is part of the ordered HKG T-24 Tmax research plan. The intended input families are: publication-safe sea temperature, station coastline distance and coast-normal vectors, terrain/elevation station context, land-use buffers with valid eras, R11 flow-relative geometry. The scientific goal is not to make a superficial model row; it is to test a specific physical or statistical mechanism under the T-1 15:00 Asia/Hong_Kong as-of contract. The required forecast target remains the official Hong Kong Observatory Headquarters daily maximum air temperature for local day T. All predictor information must be available before the cutoff, and all model-selection decisions before R30 must use development OOF evidence only.

## Current Evidence Audited

The current repository and data root contain the following relevant evidence: HKO daily sea-temperature raw files are parsed as retrospective daily climate mechanism rows, terrain, coastline, and LUHK raw packages are downloaded, station_distance_bearing exists only for static HKO/NOAA registry, not all HKO feed names. This evidence is useful but insufficient for a lawful scored experiment. The data acquisition inventory says many source families were downloaded as immutable raw payloads, but a raw payload alone is not model-ready. For R13-R30, the blocker is usually not that nothing exists; it is that the source-native parser, point-in-time issue/vintage contract, station metadata reconciliation, or complete upstream expert table is not yet available.

## Precondition Gate Result

The status for HKG-T24-R24 is `BLOCKED_PRECONDITIONS_NOT_MET`. The explicit blockers are: sea_temperature_publication_timing_unproven, hko_named_station_geospatial_context_missing, terrain_luhk_station_buffers_not_parsed, blocked_by_R11_dynamic_geometry. Because these blockers affect the actual predictor set or validation authorization, the experiment does not train a model, does not score OOF predictions, and does not promote any feature. This is a deliberate fail-closed result. It avoids fabricating features from target-day daily climate values, retrospective best tracks, current-only snapshots, unparsed raw archives, or post-hoc validation knowledge.

## Readiness Table

| category | item | status | disposition |
| --- | --- | --- | --- |
| required_input | publication-safe sea temperature | required | not scored until available |
| required_input | station coastline distance and coast-normal vectors | required | not scored until available |
| required_input | terrain/elevation station context | required | not scored until available |
| required_input | land-use buffers with valid eras | required | not scored until available |
| required_input | R11 flow-relative geometry | required | not scored until available |
| available_evidence | HKO daily sea-temperature raw files are parsed as retrospective daily climate mechanism rows | available_or_partial | audited for blocker gate |
| available_evidence | terrain, coastline, and LUHK raw packages are downloaded | available_or_partial | audited for blocker gate |
| available_evidence | station_distance_bearing exists only for static HKO/NOAA registry, not all HKO feed names | available_or_partial | audited for blocker gate |
| blocker | sea_temperature_publication_timing_unproven | blocked | Parse static geospatial station context and prove sea-temperature publication timing before testing marine/terrain interactions. |
| blocker | hko_named_station_geospatial_context_missing | blocked | Parse static geospatial station context and prove sea-temperature publication timing before testing marine/terrain interactions. |
| blocker | terrain_luhk_station_buffers_not_parsed | blocked | Parse static geospatial station context and prove sea-temperature publication timing before testing marine/terrain interactions. |
| blocker | blocked_by_R11_dynamic_geometry | blocked | Parse static geospatial station context and prove sea-temperature publication timing before testing marine/terrain interactions. |
