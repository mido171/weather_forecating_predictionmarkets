...
def test_publication_metadata_survives_raw_archive_relocation(tmp_path) -> None:
    original_root = tmp_path / "original"
    original_raw = original_root / "raw"
    source_id = "hko_daily_extract_202606"
    ...
    original_sidecar = json.loads(snapshot.sidecar_path.read_text(encoding="utf-8"))

    relocated_root = tmp_path / "relocated"
    relocated_root.mkdir()
    shutil.move(str(original_raw), str(relocated_root / "raw"))

    rows = build_daily_extract_publication_ledger(
        raw_root=relocated_root / "raw",
        year=2026,
        month=6,
        source_id=source_id,
    )
    ...
    assert len(rows) == 1
    assert rows[0].first_value == "32.0"
    assert original_sidecar["content_path"].startswith(str(original_root.resolve()))
    assert original_sidecar["content_relpath"].startswith("raw/")
