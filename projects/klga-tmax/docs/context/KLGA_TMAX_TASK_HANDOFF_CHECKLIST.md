...
- [ ] Code changes are under `src/klga_tmax`.
...
Run from `projects/klga-tmax` in the prepared project environment:
...
Run migration, contract inspection, direct DB readback, and DB-backed validation only when
a prepared local database and explicit authorization are in scope. Keep `KLGA_DB_URL` in the
ignored local environment; never place a real password in this checklist.
...
- [ ] Create `docs/context/KLGA_TMAX_<NN>_<TASK_SLUG>_IMPLEMENTATION_DEEP_DIVE.md`.
