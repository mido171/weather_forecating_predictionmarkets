# EXP-0051 / HKG-T24-R19 Long-Form Experiment Report
...
This experiment is part of the ordered HKG T-24 Tmax research plan. The intended input families are: exact reproducible baseline analogue feature table, promoted or conditionally retained R04-R18 feature families, nested rolling-origin tuning, neighbor identity audit table. The scientific goal is not to make a superficial model row; it is to test a specific physical or statistical mechanism under the T-1 15:00 Asia/Hong_Kong as-of contract. The required forecast target remains the official Hong Kong Observatory Headquarters daily maximum air temperature for local day T. All predictor information must be available before the cutoff, and all model-selection decisions before R30 must use development OOF evidence only.

## Current Evidence Audited

The current repository and data root contain the following relevant evidence: supplied baseline predictions were reproduced from row-level archive in R01, baseline rerun/hash parity remains documented as a reproduction blocker, R04-R12 diagnostics exist but none are promotable under the four-year OOF rule. This evidence is useful but insufficient for a lawful scored experiment. The data acquisition inventory says many source families were downloaded as immutable raw payloads, but a raw payload alone is not model-ready. For R13-R30, the blocker is usually not that nothing exists; it is that the source-native parser, point-in-time issue/vintage contract, station metadata reconciliation, or complete upstream expert table is not yet available.

## Precondition Gate Result

The status for HKG-T24-R19 is `BLOCKED_PRECONDITIONS_NOT_MET`. The explicit blockers are: exact_baseline_rerun_hash_mismatch, no_promoted_feature_families_from_R04_to_R18, nested_analogue_neighbor_audit_not_built. Because these blockers affect the actual predictor set or validation authorization, the experiment does not train a model, does not score OOF predictions, and does not promote any feature. This is a deliberate fail-closed result. It avoids fabricating features from target-day daily climate values, retrospective best tracks, current-only snapshots, unparsed raw archives, or post-hoc validation knowledge.

## Readiness Table

| category | item | status | disposition |
| --- | --- | --- | --- |
| required_input | exact reproducible baseline analogue feature table | required | not scored until available |
| required_input | promoted or conditionally retained R04-R18 feature families | required | not scored until available |
| required_input | nested rolling-origin tuning | required | not scored until available |
| required_input | neighbor identity audit table | required | not scored until available |
| available_evidence | supplied baseline predictions were reproduced from row-level archive in R01 | available_or_partial | audited for blocker gate |
| available_evidence | baseline rerun/hash parity remains documented as a reproduction blocker | available_or_partial | audited for blocker gate |
| available_evidence | R04-R12 diagnostics exist but none are promotable under the four-year OOF rule | available_or_partial | audited for blocker gate |
| blocker | exact_baseline_rerun_hash_mismatch | blocked | Repair exact baseline rerun parity and complete required upstream parsers before starting analogue redesign. |
| blocker | no_promoted_feature_families_from_R04_to_R18 | blocked | Repair exact baseline rerun parity and complete required upstream parsers before starting analogue redesign. |
| blocker | nested_analogue_neighbor_audit_not_built | blocked | Repair exact baseline rerun parity and complete required upstream parsers before starting analogue redesign. |
