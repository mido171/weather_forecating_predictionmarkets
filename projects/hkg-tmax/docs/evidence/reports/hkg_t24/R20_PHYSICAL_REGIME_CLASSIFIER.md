# EXP-0052 / HKG-T24-R20 Long-Form Experiment Report
...
This experiment is part of the ordered HKG T-24 Tmax research plan. The intended input families are: R07 transition features, R11 dynamic advection or fixed fallback, R13 rain/cloud suppression, R14/R15 upper-air structure, R18 official forecast signals. The scientific goal is not to make a superficial model row; it is to test a specific physical or statistical mechanism under the T-1 15:00 Asia/Hong_Kong as-of contract. The required forecast target remains the official Hong Kong Observatory Headquarters daily maximum air temperature for local day T. All predictor information must be available before the cutoff, and all model-selection decisions before R30 must use development OOF evidence only.

## Current Evidence Audited

The current repository and data root contain the following relevant evidence: R07 transition proxy diagnostic exists, R11 is blocked by missing station geometry, R13/R14/R18 are blocked by missing parsers. This evidence is useful but insufficient for a lawful scored experiment. The data acquisition inventory says many source families were downloaded as immutable raw payloads, but a raw payload alone is not model-ready. For R13-R30, the blocker is usually not that nothing exists; it is that the source-native parser, point-in-time issue/vintage contract, station metadata reconciliation, or complete upstream expert table is not yet available.

## Precondition Gate Result

The status for HKG-T24-R20 is `BLOCKED_PRECONDITIONS_NOT_MET`. The explicit blockers are: rain_cloud_regime_inputs_missing, upper_air_regime_inputs_missing, official_forecast_regime_inputs_missing, specialist_expert_training_not_eligible. Because these blockers affect the actual predictor set or validation authorization, the experiment does not train a model, does not score OOF predictions, and does not promote any feature. This is a deliberate fail-closed result. It avoids fabricating features from target-day daily climate values, retrospective best tracks, current-only snapshots, unparsed raw archives, or post-hoc validation knowledge.

## Readiness Table

| category | item | status | disposition |
| --- | --- | --- | --- |
| required_input | R07 transition features | required | not scored until available |
| required_input | R11 dynamic advection or fixed fallback | required | not scored until available |
| required_input | R13 rain/cloud suppression | required | not scored until available |
| required_input | R14/R15 upper-air structure | required | not scored until available |
| required_input | R18 official forecast signals | required | not scored until available |
| available_evidence | R07 transition proxy diagnostic exists | available_or_partial | audited for blocker gate |
| available_evidence | R11 is blocked by missing station geometry | available_or_partial | audited for blocker gate |
| available_evidence | R13/R14/R18 are blocked by missing parsers | available_or_partial | audited for blocker gate |
| blocker | rain_cloud_regime_inputs_missing | blocked | Complete the upstream parsers and then fit regime probabilities inside chronological folds with minimum sample constraints. |
| blocker | upper_air_regime_inputs_missing | blocked | Complete the upstream parsers and then fit regime probabilities inside chronological folds with minimum sample constraints. |
| blocker | official_forecast_regime_inputs_missing | blocked | Complete the upstream parsers and then fit regime probabilities inside chronological folds with minimum sample constraints. |
| blocker | specialist_expert_training_not_eligible | blocked | Complete the upstream parsers and then fit regime probabilities inside chronological folds with minimum sample constraints. |
