...
EXPERIMENT_ID = "0191"
SLUG = "t7_sign_asymmetric_residual_calibration"
TITLE = "T-7 Sign-Asymmetric Residual Calibration"
EXP_DIR = EXPERIMENTS_ROOT / f"{EXPERIMENT_ID}_{SLUG}"
PRIMARY_CANDIDATE_ID = "0191_t7_sign_asymmetric_champion_residual_calibrator"
P0190 = EXPERIMENTS_ROOT / "0190_t7_post_ensemble_residual_calibration" / "predictions.parquet"
SRC_COPY_NAME = "run_0191.py"
MODEL_FOLDS = base.MODEL_FOLDS
LAG_DAYS = 7
CONFIG_GRID = [
    {"context_mode": "global_sign", "halflife": 10.0, "min_history": 10, "shrink": 40.0, "cap_c": 0.08},
    {"context_mode": "global_sign", "halflife": 20.0, "min_history": 10, "shrink": 70.0, "cap_c": 0.10},
    {"context_mode": "source_sign", "halflife": 20.0, "min_history": 12, "shrink": 70.0, "cap_c": 0.12},
    {"context_mode": "season_sign", "halflife": 30.0, "min_history": 15, "shrink": 80.0, "cap_c": 0.12},
    {"context_mode": "month_sign", "halflife": 45.0, "min_history": 20, "shrink": 100.0, "cap_c": 0.15},
    {"context_mode": "all_sign", "halflife": 45.0, "min_history": 20, "shrink": 100.0, "cap_c": 0.15},
    {"context_mode": "all_sign_mag", "halflife": 45.0, "min_history": 20, "shrink": 100.0, "cap_c": 0.18},
    {"context_mode": "all_sign_mag", "halflife": 90.0, "min_history": 30, "shrink": 140.0, "cap_c": 0.20},
]
...
def contexts(row: pd.Series, mode: str) -> list[str]:
    ...
    out = ["global", f"global|{corr_sign}"]
    if mode in {"source_sign", "all_sign", "all_sign_mag"}:
        out.extend([f"source={source}", f"source={source}|{corr_sign}"])
    if mode in {"season_sign", "all_sign", "all_sign_mag"}:
        out.extend([f"season={season}", f"season={season}|{corr_sign}", f"season={season}|source={source}|{corr_sign}"])
    if mode in {"month_sign", "all_sign", "all_sign_mag"}:
        out.extend([month, f"{month}|{corr_sign}", f"{month}|source={source}|{corr_sign}"])
...
def run_walk_forward(frame: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    out["model_family"] = "t7_sign_asymmetric_residual_calibration"
...
def build_spec(created_at: str) -> dict[str, Any]:
    return {
        ...
        "hypothesis": "The 0190 champion still has correction-sign-conditioned residual bias that can be reduced by a second-stage T-7 online residual calibrator.",
        "rationale": "0190 is the current comparable champion but remaining residual anatomy shows month and correction-sign asymmetry. A small mature residual-state correction tests whether that structure is persistent and deployable.",
        "expected_sign_and_falsification": "Expected sign is lower MAE than 0190 with no tail harm. Falsified if sign-asymmetric calibration overcorrects stale noise or damages the weak June slice.",
        "novelty": {"prior_experiments": ["0189", "0190"], "difference": "Post-champion residual calibration of the selected 0190 predictions using known correction sign and magnitude contexts.", "similarity_audit_path": "RESULTS.md#comparison-limitations"},
        ...
        "features": {"generation_rule": "EW residual states by global/source/season/month plus known 0190 correction sign and magnitude contexts, updated only from 0190 residuals matured by T-7.", "grid": CONFIG_GRID, "explicit_exclusions": ["2024+ rows", "current target residual", "residuals newer than T-7"]},
        "response": {"variable": "target_tmax_c - parent_0190_prediction_c", "prediction": "parent_0190_prediction_c plus clipped mature sign-asymmetric residual correction"},
...
Experiment `{EXPERIMENT_ID}` is `{summary['status']}`. It is a second-stage T-7 sign-asymmetric residual calibrator over 0190.

## One-Sentence Hypothesis

The remaining 0190 champion residual can be reduced by mature prior states keyed to known correction sign and magnitude without violating T-24.

## Why It Is Worth Doing

0190 is the current comparable champion but still leaves month and correction-sign residual structure. A small calibrator directly tests whether that structure is persistent.

## Prior Evidence And Novelty

0189 established online expert weighting and 0190 established post-ensemble calibration. 0191 tests whether known correction-sign contexts add residual information beyond that champion.
...
Features are mature residual states by global, source, season, month, and known 0190 correction sign/magnitude contexts. Details are in `feature_definitions.csv`.
...
Calibration can fail by overcorrecting stale bias or duplicating memory already inside 0190.

## Reproduction Command

Run `python scripts/run_hkg_t24_0191_t7_sign_asymmetric_residual_calibration.py` from the repository root.
""")
...
0191 tested whether 0190's remaining correction-sign-conditioned residual bias was stable enough for another mature residual correction.
...
If not promoted, the likely cause is residual-memory redundancy with 0190's own post-ensemble calibration.
...
def main() -> None:
    ...
    scoreboard = pd.DataFrame(
        [
            ...
            {"candidate_id": PRIMARY_CANDIDATE_ID, "model_family": "t7_sign_asymmetric_residual_calibration", "n": candidate_global["n"], "mae_c": candidate_global["mae_c"], "rmse_c": candidate_global["rmse_c"], "bias_c": candidate_global["bias_c"], "median_abs_error_c": candidate_global["median_abs_error_c"], "p95_abs_error_c": candidate_global["p95_abs_error_c"], "gt2c_rate": candidate_global["gt2c_rate"], "gt3c_rate": candidate_global["gt3c_rate"], "baseline_mae_c": official_global["mae_c"], "mae_delta_c": mae_delta},
    ...
    data_manifest = pd.DataFrame([{"source_id": "0190_parent_predictions", "path": rel(P0190), "sha256": sha256_file(P0190), "size_bytes": P0190.stat().st_size, "row_count": int(len(frame)), "date_start": date_text(frame["target_date"].min()), "date_end": date_text(frame["target_date"].max()), "timestamp_fields": "target_date;T-7 parent residual state;parent correction sign", "availability_class": "DEPLOYABLE_LAGGED_ONLY", "notes": "Validator-clean parent 0190 predictions."}])
    feature_definitions = pd.DataFrame([{"feature_name": "t7_parent_0190_sign_asymmetric_residual_calibration", "role": "candidate_correction", "formula": "EW mean of parent 0190 residuals in contexts keyed by source season month and known parent correction sign or magnitude; only rows with target_date <= T-7 are admitted.", "input_columns": "parent_0190_prediction_c,parent_0190_calibration_correction_c,target_tmax_c,season,month,source", "units": "degC", "lag": "7 target days minimum", "window": "predeclared EW half-life grid", "fit_scope": "fold-local config selection; prequential state", "availability_rule": "No current or recent target residual updates prediction state; current known parent correction sign is not target-derived for the current row.", "missingness_policy": "No mature state gives zero correction."}])
...
0191 consumes validator-clean 0190 parent predictions. Calibration states use parent residuals only after target dates are at least `{LAG_DAYS}` days old. The current-row context may use the known parent correction sign and magnitude, but never the current target residual.
...
```powershell
python scripts/run_hkg_t24_0191_t7_sign_asymmetric_residual_calibration.py
```
