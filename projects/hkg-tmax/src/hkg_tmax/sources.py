...
def write_source_inventory(root: Path, catalog: SourceCatalog) -> Path:
    ...
    lines = [
        "# Source Inventory",
        "",
        "Generated from `config/sources/data_sources.yaml`. Endpoint implementation and source-contract "
    ...
    lines.extend(
        [
            ...
            "",
            "For each implemented source, create a source contract under `docs/specifications/source-contracts/` "
            "and verify its official endpoint, timestamps, cadence, revision policy, terms, and tests.",
            "",
        ]
    )
    path = ProjectPaths.from_project_root(root).run_root / "reports" / "source_inventory.md"
