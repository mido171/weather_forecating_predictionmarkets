...
from hkg_tmax.paths import ProjectPaths, find_project_root
...
PROJECT_PATHS = ProjectPaths.from_project_root(REPO_ROOT)
...
EXTERNAL_HF_ROOT = PROJECT_PATHS.data_root


def require_generated_roadmap_artifacts() -> None:
    if not ROADMAP_GLOBAL_ARTIFACT_DIR.is_dir():
        pytest.skip(
            "Generated roadmap artifacts are external run state; execute the bounded roadmap "
            f"workflow first under {EXPERIMENT_ROOT}"
        )
...
def test_generated_0105_to_0183_folders_have_clean_required_layout() -> None:
    require_generated_roadmap_artifacts()
    _, specs = parse_roadmap()
