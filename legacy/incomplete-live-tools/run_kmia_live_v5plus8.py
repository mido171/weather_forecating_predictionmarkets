...
def _ensure_dependencies() -> None:
    ...
    raise RuntimeError(
        ...
        + ", ".join(missing)
        + ". Install them explicitly in a project virtual environment."
    )
...
def main() -> int:
    ...
    db_url: str | URL
    configured_url = os.getenv("MYSQL_URL", "").strip()
    if configured_url:
        db_url = configured_url
    else:
        password = os.getenv("MYSQL_PASSWORD", "")
        if not password:
            raise RuntimeError("MYSQL_URL or MYSQL_PASSWORD is required")
        db_url = URL.create(
            "mysql+pymysql",
            username=os.getenv("MYSQL_USER", "weather_app"),
            password=password,
