...
from .acquisition_contracts import write_all_acquisition_contract_outputs
...
from .source_normalization import normalize_non_minute_sources_to_output
...
def _require_execute(enabled: bool, operation: str) -> None:
    if not enabled:
        raise ValidationError(
            f"{operation} is a network operation and is dry-run by default; "
            "re-run with --execute after reviewing its scope and budget"
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sources_fetch.add_argument("--max-sources", type=int, default=1)
    sources_fetch.add_argument("--execute", action="store_true")
    ...
    map_parser.add_argument(
        "--buckets",
        default="config/project/example_market_buckets.yaml",
    ...
    collect_parser.add_argument("--max-sources", type=int, default=1)
    collect_parser.add_argument("--execute", action="store_true")
    run_due_parser = acquisition_sub.add_parser(
        "run-due", help="Run due sources from config/acquisition/collector_schedules.yaml"
    )
    run_due_parser.add_argument("--execute", action="store_true")
    ...
    acquisition_sub.add_parser(
        "contracts", help="Refresh historical/live pair and gridded acquisition contract reports"
    )
    static_context_parser = acquisition_sub.add_parser(
        "static-context", help="Build deterministic static station context metadata"
    )
    static_context_parser.add_argument("--solar-year", type=int)
    acquisition_sub.add_parser("health", help="Refresh reports/live_collector_health.md")
    normalize_parser = acquisition_sub.add_parser(
        "normalize-non-minute",
        help="Parse and normalize acquired non-minute raw source families without experiments",
    )
    normalize_parser.add_argument(
        "--input-data-root",
        default=None,
        help="Raw acquisition data root to read; defaults to configured HKG_TMAX_DATA_ROOT.",
    )
    normalize_parser.add_argument(
        "--output-data-root",
        default=None,
        help="Derived data root to write; defaults to repository data/ for sandbox-safe local runs.",
    )
    ...
    hko_backfill_parser.add_argument(
        ...
        choices=(
            ...
            "radar-lightning",
            "satellite-current",
            "datagov-historical-live",
            "datagov-historical-rss",
            "arwf-current",
            "noaa-isd-nearby",
    ...
    hko_backfill_parser.add_argument("--max-requests", type=int, default=1)
    hko_backfill_parser.add_argument("--execute", action="store_true")
    hko_backfill_parser.add_argument(
        "--skip-existing-successes",
        action="store_true",
        help="Skip URLs that already have successful rows in the acquisition ledger.",
    )
    ...
    try:
        ...
        if args.command == "sources":
            ...
            if args.sources_command == "fetch":
                ...
                if args.max_sources < 1 or requested_count > args.max_sources:
                    raise ValidationError(
                        f"Requested {requested_count} source(s), exceeding "
                        f"--max-sources={args.max_sources}"
                    )
        ...
        if args.command == "acquisition":
            ...
            if args.acquisition_command == "collect":
                _require_execute(args.execute, "acquisition collect")
                if args.max_sources < 1 or len(args.source_id) > args.max_sources:
                    raise ValidationError(
                        f"Requested {len(args.source_id)} source(s), exceeding "
                        f"--max-sources={args.max_sources}"
                    )
            ...
            if args.acquisition_command == "run-due":
                _require_execute(args.execute, "acquisition run-due")
            ...
            if args.acquisition_command == "reports":
                for path in write_inventory_reports(root):
                    print(path)
                for path in write_all_acquisition_contract_outputs(root):
                    print(path)
                return
            if args.acquisition_command == "contracts":
                for path in write_all_acquisition_contract_outputs(root):
                    print(path)
                return
            if args.acquisition_command == "static-context":
                outputs = build_static_context(root, solar_year=args.solar_year)
                print(outputs.station_registry_csv)
                print(outputs.station_distance_csv)
                print(outputs.solar_geometry_csv)
                print(outputs.report_path)
                return
            if args.acquisition_command == "health":
                print(write_health_report(root))
                return
            if args.acquisition_command == "normalize-non-minute":
                input_data_root = (
                    Path(args.input_data_root).expanduser().resolve()
                    if args.input_data_root
                    else ensure_data_root(root)
                )
                output_data_root = (
                    Path(args.output_data_root).expanduser().resolve()
                    if args.output_data_root
                    else root / "data"
                )
                normalization_outputs = normalize_non_minute_sources_to_output(
                    root=root,
                    input_data_root=input_data_root,
                    output_data_root=output_data_root,
                )
                print(normalization_outputs.manifest_path)
                print(normalization_outputs.report_path)
                for table in normalization_outputs.tables:
                    print(
                        json.dumps(
                            {
            ...
            if args.acquisition_command == "hko-backfill":
                _require_execute(args.execute, "acquisition hko-backfill")
