...
**Acquisition specs assumed implemented:** `../data-acquisition`.  
...
|---|---:|---|
| `KLGA_DB_URL` | yes | SQLAlchemy URL, e.g. `postgresql+psycopg://<user>:<password>@host:5432/weather_markets`. |
| `KLGA_ARTIFACT_ROOT` | yes | External root for model/report artifacts; normally `${KLGA_RUN_ROOT}/artifacts`. |
| `KLGA_ENV` | yes | One of `local`, `paper`, `prod`. |
| `KLGA_TRADING_MODE` | yes | One of `backtest`, `paper`, `live`. Safe runtime default is `backtest`; paper must be selected explicitly. |
...
```text
${KLGA_ARTIFACT_ROOT}/frozen_configs/frozen_config_<timestamp>_<gitsha>.json
```
...
${KLGA_ARTIFACT_ROOT}/backtests/<run_id>/
```
...
Outputs under `${KLGA_ARTIFACT_ROOT}/reports/...` and `reports.metrics`.
...
```text
Backtest mode by default; paper trading requires explicit selection.
